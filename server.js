...
const STUDIO_PASSWORD_HASH = 'eea5cdcdf82bf57d9f55839c4c71750185b9854eae92dfcab4421fc762ed5245'; // live2026 (افتراضي)
...
const STUDIO_SESSIONS = new Map(); // token -> { createdAt } (جلسات البث)
...
/* كلمة مرور البث: من الـdata.json أو الافتراضية */
function currentStudioPasswordHash() {
    return (db.settings && db.settings.studioPasswordHash) || STUDIO_PASSWORD_HASH;
}
...
function isStudioAuthed(req) {
    const auth = req.headers.authorization || '';
    const token = auth.replace(/^Bearer\s+/i, '');
    return SESSIONS.has(token) || STUDIO_SESSIONS.has(token);
}
...
function attachWebSocket(server) {
    server.on('upgrade', (req, socket) => {
        try {
            ...
            const isAdminConn = SESSIONS.has(token) || STUDIO_SESSIONS.has(token);
                    ...
                    if (opcode === 0x1) {
                        try {
                            ...
                            if (msg && msg.type === 'join' && msg.mode === 'admin' && !isAdminConn) {
                                wsSend(socket, { type: 'error', error: 'غير مصرح — أدخل كلمة مرور البث' });
                                close();
                                return;
                            }
...
function handleApi(req, res, url) {
    ...
    // POST /api/studio-login  ->  { password }  (دخول استوديو البث بكلمة مستقلة)
    if (method === 'POST' && pathname === '/api/studio-login') {
        return readBody(req, body => {
            if (!body.password) return sendJson(res, 400, { ok: false, error: 'كلمة المرور مطلوبة' });
            const hash = crypto.createHash('sha256').update(body.password).digest('hex');
            if (hash !== currentStudioPasswordHash()) {
                return sendJson(res, 401, { ok: false, error: 'كلمة المرور غير صحيحة' });
            }
            const token = createToken();
            STUDIO_SESSIONS.set(token, { createdAt: Date.now() });
            return sendJson(res, 200, { ok: true, token });
        });
    }

    // POST /api/change-studio-password  ->  { currentPassword, newPassword } (محمي للأدمن)
    if (method === 'POST' && pathname === '/api/change-studio-password') {
        ...
        return readBody(req, body => {
            ...
            if (curHash !== currentStudioPasswordHash()) {
            ...
            db.settings.studioPasswordHash = newHash;
            saveData(db);
            STUDIO_SESSIONS.clear();
            return sendJson(res, 200, { ok: true, message: 'تم تغيير كلمة مرور البث' });
        });
    }
    ...
    if (method === 'POST' && pathname === '/api/recordings/upload') {
        if (!isStudioAuthed(req)) return sendJson(res, 401, { ok: false, error: 'غير مصرح' });
